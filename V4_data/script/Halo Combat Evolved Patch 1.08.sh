...
if [ "$POL_LANG" == "fr" ]; then
LNG_GAME_UPDATE_WELCOME="Bienvenue dans le script d'installation du patch $VERSION pour $TITLE"
...
LNG_GAME_UPDATE_WELCOME="Welcome in the patch $VERSION Installation script for $TITLE"
...
if [ "$APP_ANSWER" == "$LNG_HAVE_PATCH" ]; then
	...
	POL_SetupWindow_download "$LNG_GAME_UPDATE_DL" "$TITLE" "http://www.bungie.net/images/games/halopc/patch/halopc108patch.exe"
	wine start /unix "halopc108patch.exe"
	rm "halopc108patch.exe"
fi
...
iEYEABECAAYFAk43+yAACgkQ5TH6yaoTykcRCwCfUeAYbwQ1YqHagh1MnkMhJgHl
N6MAn3KEYKAzWrywADVjB8LVjU+J+DBc
=FByc
-----END PGP SIGNATURE-----
