...
#
# TESTED Editions: - retail CD-ROM v1.0.0 (I think) and the date of k2.exe on the CD is 2004-08-05.
#                  - retail CD-ROM v1.2.3 (saw in version.txt into the .cab file) and the date of k2.exe on the CD is 2009-09-24 (budget release ?).
#
# Middlewares used by this software : DirectX 9.0b .
#
# CHANGELOG
# [Dadu042] (2020-01-05)
#   Initial script (Wine 4.0.3).
#
# KNOWN ISSUES:
#  - (CD v1.0.0) Wine x86 4.0.3, 3.0.3 : installer does launch but nothing is visible (I has to kill the process 'IDriver.exe'). Tried: gdiplus. copying 'VILLAGE.TTF' and 'LBRITED.TTF' into /windows/fonts/
#  - (CD v1.2.3) Wine x86 4.0.3, 3.0.3, 4.21, 5.0-rc3, 3.20 : installer does not launch but nothing is visible (however it does close itself after some seconds). Tried: copying 'VILLAGE.TTF' and 'LBRITED.TTF' into /windows/fonts/
#
# KNOWN ISSUES (FIXED):
#  - Wine amd64 4.0.3: X.
...
PREFIX="kohan_II_KoW"
EDITOR="Timegate studios"
GAME_URL="https://en.wikipedia.org/wiki/Kohan_II:_Kings_of_War"
AUTHOR="Dadu042"
STEAM_ID=""
GAME_VMS="128"
SHORTCUT_FILENAME="k2.exe"
...
############################
# Add missing fonts update #
############################

# from: https://appdb.winehq.org/objectManager.php?sClass=version&iId=9042

POL_SetupWindow_menu "$(eval_gettext 'Want to fix this ?: ERROR: Font family not found for Fonts/TrueType/LBRITED.TTF ')" "$TITLE" "$(eval_gettext 'Yes')~$(eval_gettext 'No')" "~"

if [ "$APP_ANSWER" == "$(eval_gettext 'Yes')" ]; then

        POL_SetupWindow_browse "$(eval_gettext 'Please select a file LBRITED.TTF (download it or get from somewhere).')" "$TITLE"
        SETUP_EXE="$APP_ANSWER"

	cp "$APP_ANSWER" "$WINEPREFIX/drive_c/windows/fonts/"
fi
...
if [ "$INSTALL_METHOD" == "STEAM" ]; then
        ...
        POL_SetupWindow_check_cdrom "K2.exe"
        ...
        POL_Download "http://www.armadillorun.com/demo/Armadillo_Run_Demo_1_0_7.exe"
        # POL_SetupWindow_message "$(eval_gettext 'Note: we recommend you to uncheck all the checkboxes:\n[x] -> [ ]')" "$TITLE"

        POL_Wine "Armadillo_Run_Demo_1_0_7.exe" # "/SILENT"
...
iF0EABECAB0WIQRFtWEU2eoWQNaBNczlMfrJqhPKRwUCXhMejgAKCRDlMfrJqhPK
R1asAKCxk0vGHH98ccAKPJJ++wuisTDEcACdGCPb8cY3B1aMyYMlbAyveiww4Vc=
=Vh9Q
-----END PGP SIGNATURE-----
