...
#
# TESTED Editions: release from archive.org (2016).
#
...
# [Dadu042] (2019-11-22 15:15)
#   Add a Known issue.
#   Add creating folder 'idinaloq'.
#
# KNOWN ISSUES:
#  - Wine amd64 4.0.2, 4.0.3, 4.19, 4.21, 5.0, 6.0, 7.0: freeze on the screen 'Now Loading' (after the main menu).
#  - Wine x86 4.0.2: same as above.
#  - Wine x86/amd64 4.0.2: on the character selection screen (before 'Now Loading') the 3D starships are not displayed. Fix: copy/paste 'd3drm.dll'
#
# Ideas to improve this script: select archive, then decide if extension is RAR or ZIP or 7Z...
 
# Perhaps useful (d3drm):
# https://www.dlldump.com/download-dll-files_new.php/dllfiles/D/d3drm.dll/5.1.2600.0/download.html
...
TITLE="Idinaloq"
PREFIX="Idinaloq"
EDITOR="Namikaze Project"
GAME_URL="https://www.namikaze.org/software/idinaloq/"
AUTHOR="Dadu042"
STEAM_ID=""
WORKING_WINE_VERSION="7.0"
GAME_VMS="256"
SHORTCUT_FILENAME="idinaloq.exe"
...
if [ "$INSTALL_METHOD" == "STEAM" ]; then
        ...
        mkdir "idinaloq"
        POL_SetupWindow_message "$(eval_gettext 'Note: the download is possible thank to the project Archive.org')" "$TITLE"
        POL_Download "https://archive.org/download/idinaloq_by_namikazi_project_hi_quality_sound_and_movies/idinaloq_by_namikazi_project_hi_quality_sound_and_movies.zip"
        # SETUP_EXE="$APP_ANSWER"
        # cd "$POL_System_TmpDir"
        POL_SetupWindow_wait_next_signal "$(eval_gettext 'Extracting the archive...')" "$TITLE"
        POL_System_unzip "idinaloq_by_namikazi_project_hi_quality_sound_and_movies.zip" -d "$WINEPREFIX/drive_c/idinaloq/"
         
        # POL_Wine "idinaloq.exe" "/SILENT"
        # POL_Wine_WaitBefore "$TITLE"
 
        # Restore screen resolution (game's default is 5xx X 4xx)
        POL_Shortcut_InsertBeforeWine "$SHORTCUT" "trap 'xrandr -s 0' EXIT"
...
if [ "$APP_ANSWER" == ".EXE" ]; then
        ...
        cd "$POL_System_TmpDir"
        POL_SetupWindow_wait_next_signal "$(eval_gettext 'Extracting the archive...')" "$TITLE"
        POL_System_unzip "$APP_ANSWER" -d "$WINEPREFIX/drive_c/idinaloq/"
        POL_Shortcut "$SHORTCUT_FILENAME" "$TITLE" "" "" "$SOFTWARE_CATEGORIES"
...
iF0EABECAB0WIQRFtWEU2eoWQNaBNczlMfrJqhPKRwUCYhVtzwAKCRDlMfrJqhPK
R5TsAJ9xq8Dmgd5JhWBiufVhzlg8cZwiMQCffewhbIJ2mGfIJpT2o8Ea7EUswCk=
=Jrnl
-----END PGP SIGNATURE-----
