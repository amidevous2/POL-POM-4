...
#
# TESTED Editions: local ZIP v0.5
#
...
#
# KNOWN ISSUES:
#  - Wine x86 4.0.3: mouse does not work in the window.
#
...
PREFIX="Previous"
EDITOR=""
GAME_URL="https://en.wikipedia.org/wiki/Previous_(software)"
AUTHOR="Dadu042"
STEAM_ID=""
WORKING_WINE_VERSION="4.0.3"
GAME_VMS="256"
SHORTCUT_FILENAME="previous.exe"
...
if [ "$INSTALL_METHOD" == "STEAM" ]; then
        ...
        POL_SetupWindow_message "$(eval_gettext '\nNote: this script will download the v0.5 (2015).')" "$TITLE"

        cd "$WINEPREFIX/drive_c"
        POL_Download "http://previous.alternative-system.com/files/previous_0_5_win32.zip"

        POL_SetupWindow_wait_next_signal "$(eval_gettext 'Extracting the archive...')" "$TITLE"
        POL_System_unzip "previous_0_5_win32.zip" -d "$WINEPREFIX/drive_c/previous/"
...
if [ "$APP_ANSWER" == ".EXE" ]; then
        ...
        POL_SetupWindow_wait_next_signal "$(eval_gettext 'Extracting the archive...')" "$TITLE"
        POL_System_unzip "$APP_ANSWER" -d "$WINEPREFIX/drive_c/previous/"
        POL_Shortcut "$SHORTCUT_FILENAME" "$TITLE" "" "" "$SOFTWARE_CATEGORIES"
...
iF0EABECAB0WIQRFtWEU2eoWQNaBNczlMfrJqhPKRwUCXfSTTwAKCRDlMfrJqhPK
R18dAJ4oABorc3fU5Lg7R2/4wcOUj5cL9gCeNeyqClgEY1VRKaFkFKynjky78y0=
=zdDX
-----END PGP SIGNATURE-----
