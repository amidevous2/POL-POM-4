...
#
# TESTED Editions: demo only.
#
# Middlewares used by this software : .
#
# CHANGELOG
# [Dadu042] (2020-01-17)
#   First script (game can work without Alsa).
#
# KNOWN ISSUES:
#  - Wine amd64 4.0.3: sounds missing when playing. Fix: dsound.
#  - Wine amd64 4.0.3: sound of the intro is NOK (clicks) but become OK in the main menu (and issue again when the game is playing). Tried: alsa, amstream, quartz. Fix: devenum.
...
TITLE="DemonStar: Secret Missions 2"
PREFIX="demonstar2"
EDITOR="Moutain King Studios Inc."
GAME_URL="http://mking.com"
AUTHOR="Dadu042"
STEAM_ID=""
WORKING_WINE_VERSION="3.0.3"
GAME_VMS="256"
SHORTCUT_FILENAME="ds.exe"
...
if [ "$INSTALL_METHOD" == "STEAM" ]; then
        ...
        # POL_SetupWindow_message "$(eval_gettext '\n\nNote: this script can only download the demo (called shareware).')" "$TITLE"
        POL_Download "http://www.mking.com/download/demonstarsm2/demonsm2shr.exe"

        # POL_SetupWindow_wait_next_signal "$(eval_gettext 'Extracting the archive...')" "$TITLE"
        # POL_System_unzip "armoredfist3demo.zip" -d "$WINEPREFIX/drive_c/game/"
                 
 
        # POL_SetupWindow_message "$(eval_gettext 'Note: we recommend you to uncheck all the checkboxes:\n[x] -> [ ]')" "$TITLE"
 
	# cd  "$WINEPREFIX/drive_c/game/"
        POL_Wine "demonsm2shr" # "/SILENT"
        POL_Wine_WaitBefore "$TITLE"

        POL_Shortcut "ds.exe" "$TITLE (Demo)" "" "" "$SOFTWARE_CATEGORIES"
...
iF0EABECAB0WIQRFtWEU2eoWQNaBNczlMfrJqhPKRwUCXiHe7gAKCRDlMfrJqhPK
R/l0AJ0VjsSJ6PSnB3t9BUllpwxYwBX/SgCbBmnedQPgrSFwa5mZkTZRR72mzc4=
=wghB
-----END PGP SIGNATURE-----
