...
#
# KNOWN ISSUES:
#  - Wine amd64 3.0.3: fail to restore the OS desktop screen resolution (1368x768), even if using the command xrandr.
...
TITLE="Cloud (2005)"
PREFIX="Cloud2005"
EDITOR="USC Interactive Media Division"
GAME_URL="https://en.wikipedia.org/wiki/Cloud_(video_game)"
AUTHOR="Dadu042"
STEAM_ID=""
WORKING_WINE_VERSION="3.0.3"
GAME_VMS="256"
SHORTCUT_FILENAME="Cloud.exe"
...
if [ "$INSTALL_METHOD" == "STEAM" ]; then
        ...
        POL_Download "https://archive.org/download/Cloud_game/Cloud.rar"
        # SETUP_EXE="$APP_ANSWER"
        # cd "$POL_System_TmpDir"

        POL_SetupWindow_wait_next_signal "$(eval_gettext 'Extracting the archive...')" "$TITLE"
        POL_System_unrar x "Cloud.rar" "$WINEPREFIX/drive_c/" || POL_Debug_Fatal "unrar is required to unarchive $TITLE (unrar package is not installed on the OS)."

        POL_Wine "Cloudinstall.exe" "/SILENT"
        POL_Wine_WaitBefore "$TITLE"

        # Restore screen resolution (game's default is 1024x768)
        POL_Shortcut_InsertBeforeWine "$SHORTCUT_FILENAME" "trap 'xrandr -s 0' EXIT"
...
iF0EABECAB0WIQRFtWEU2eoWQNaBNczlMfrJqhPKRwUCXbcEjwAKCRDlMfrJqhPK
R1lrAJ9wBm0PHzY8TDpWZjeHZsQqd3qzSwCcD0rhLVvclGrM6TNm1T34XSt53MM=
=Spke
-----END PGP SIGNATURE-----
