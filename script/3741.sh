...
# [Dadu042] (2019-12-12)
#   First script.
# [Dadu042] (2020-09-27). AppleWin1.29.13.0.zip
#   Upgrade the version of the download link.
#   Wine 4.0.3 -> 4.0.4
#
#
# KNOWN ISSUES:
#  - Wine x86 4.0.4, 5.0.2: when in full screen mode the right icons only appears when the mouse cursor moves over.
#
...
PREFIX="AppleWin"
EDITOR="Mike O'Brien"
GAME_URL="https://en.wikipedia.org/wiki/AppleWin"
AUTHOR="Dadu042"
STEAM_ID=""
WORKING_WINE_VERSION="4.0.4"
GAME_VMS="256"
SHORTCUT_FILENAME="Applewin.exe"
...
if [ "$INSTALL_METHOD" == "STEAM" ]; then
        ...
        POL_SetupWindow_message "$(eval_gettext '\nNote: this script will download the v1.29.13 (2020).')" "$TITLE"
 
        cd "$WINEPREFIX/drive_c"
        POL_Download "https://github.com/AppleWin/AppleWin/releases/download/v1.29.13.0/AppleWin1.29.13.0.zip"
 
        POL_SetupWindow_wait_next_signal "$(eval_gettext 'Extracting the archive...')" "$TITLE"
        POL_System_unzip "AppleWin1.29.13.0.zip" -d "$WINEPREFIX/drive_c/applewin/"
        ...
        POL_Shortcut "$SHORTCUT_FILENAME" "$TITLE" "" "" "$SOFTWARE_CATEGORIES"
        # POL_Shortcut_Document "$TITLE" "Applewin.chm"
...
if [ "$APP_ANSWER" == ".EXE" ]; then
        ...
        POL_SetupWindow_wait_next_signal "$(eval_gettext 'Extracting the archive...')" "$TITLE"
        POL_System_unzip "$APP_ANSWER" -d "$WINEPREFIX/drive_c/applewin/"
        POL_Shortcut "$SHORTCUT_FILENAME" "$TITLE" "" "" "$SOFTWARE_CATEGORIES"
...
iFwEABECAB0WIQRFtWEU2eoWQNaBNczlMfrJqhPKRwUCX3DPUAAKCRDlMfrJqhPK
R17TAJ92mUZw1pVuCh3eexpiwYEMq0ffSwCWOjA42LaoQXmNCDdQQGLw5bWzzw==
=RhWH
-----END PGP SIGNATURE-----
