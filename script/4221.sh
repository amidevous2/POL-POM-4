...
#
# TESTED Editions (Glyph's window: button Settings->General): 'Version: STABLE-103-1842-A-284719' (2020-09).
#                                                             'Version: STABLE-103-1856-A-288318' (2020-10-22).
#
# Middlewares used by this software : QT5, vcr110 (vcrun2012), vcr120 (vcrun2013), vcruntime140 (vcrun2015).
#
#
# CHANGELOG
# [Dadu042] (2020-09-24 10-00).
#   Initial script.
# [Dadu042] (2020-10-22 10-00).
#   Add comments (I haveve try to run Defiance).
#
# KNOWN ISSUES :
#  - Wine amd64 5.0.2: in the game Trove, missing text characters in the box Settings. Fix: in 'Video setting' force DirectX to OpenGL.
#  - Wine amd64 5.0.2: in the game Trove, full screen does not work when in setting OpenGL (while Ok in DirectX, 1280x1024), instead I get a window.
#  - Wine amd64 5.0.2: in the game Trove, after playing 5 min I decided in the settings to change the Rendering from DirectX to OpenGL. When trying to restart the game, almost impossible because of multiple (25) error windows ('OpenGL Error. Shader Object ...  error: #extension directive is not allowed in the middle of a shader').
#
#  - Wine amd64 5.0.2, 5.16: in the game Trove, log 'GlyphClient::LoadFile error: No such file or directory GlyphClientApp.xml'. Because missing tweaked vcrun2015 ? Tried: xmllite.
#
#  - Wine amd64 5.0.2, 5.10, 5.16: in the game ArcheAge, hitting Play button does not launch the game (in the debug log of Wine 5.0.2 there is a crash: 'stack overflow', nothing with Wine 5.16). Game version: history.txt = 'version 507265' 
#                      Perhaps it's because this game use Easy Anti Cheat (files are into the game folder).
#  - Wine amd64 5.11-staging: in the game ArcheAge, hitting Play button does not launch the game (error window 'WinLicense: An Error has ocurred while loading imports. Wrong DLL present'). Game version: history.txt = 'version 507265' 
#
#  - Wine amd64 5.0.2: when Glyph does open, there is a background crash window.
#
#  Defiance.exe file version: 'Date signed 10:11 AM 2/15/2020'. Uses vcr110 (vcrun2010).
#  - (Defiance game) Wine amd64 5.0.2, 5.11, 5.11-staging, 5.17, 5.19, 5.19-staging, 4.21, 4.21-staging: after the intro cutscene (where the space ship does crash), the loading screen appears but the loading seems to freeze ('Loading in progress'). Tried: d3dcompiler_47, d3dx9_47, d3dx9, DXVK_172, wininet.
#  - (Defiance game) Wine amd64 5.0.2: 'GlyphClient::LoadFile error: No such file or directory GlyphClientApp.xml' and also for 'GlyphClientAppLocal.xml'. Tried: xmllite. Minor issue (ref: http://forums.trovegame.com/showthread.php?135501-Wine-linux-Cant-update-launch-trove)
#  - (Defiance game) Wine amd64 5.0.2: '# 0278:err:winsock:interface_bind_check No cache entry for ifindex 0.' Fix: wininet.
#
#
# KNOWN ISSUES (FIXED):
#  - Wine amd64 5.0.2: X
...
TITLE="Glyph (game store)"
PREFIX="Glyph"
EDITOR="Trion Worlds"
GAME_URL="https://store.trionworlds.com/"
AUTHOR="Dadu042"
STEAM_ID=""
GAME_VMS="512"
SHORTCUT_FILENAME="GlyphClient.exe"
...
if [ "$APP_ANSWER" == "64 bits (recommended)" ]; then
...
if [ "$INSTALL_METHOD" == "STEAM" ]; then
        ...
        POL_Download "http://download.dyn.triongames.com/GlyphInstall-0-1.exe"
           
        mv GlyphInstall-0-1.exe GameInstaller.exe
...
iF0EABECAB0WIQRFtWEU2eoWQNaBNczlMfrJqhPKRwUCX5Gi0QAKCRDlMfrJqhPK
R6nfAJoCAIr4mxgLEVrplYJiQs6qEM+3RQCeMW/N00oa6rEvj5ulO7f0Z0DnACE=
=tOjA
-----END PGP SIGNATURE-----
