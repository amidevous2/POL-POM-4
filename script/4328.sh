...
#
# TESTED Editions: v1.06
#
# Middlewares used by this software : 
#
# CHANGELOG
# [Dadu042] (2021-02-15 10-00).
#   Initial script.
#   Still stuck on the 'Install translation' script part.
#
# KNOWN ISSUES :
#  - Wine amd64 4.21, 5.0.3: X
#
# KNOWN ISSUES (FIXED):
#  - Wine amd64 5.0.3: X
  
...
PREFIX="TheSpiritEngine2"
EDITOR=""
GAME_URL=""
AUTHOR="Dadu042"
STEAM_ID=""
GAME_VMS="256"
SHORTCUT_FILENAME="The Spirit Engine 2.exe"
...
        POL_Download "http://tse.crypt.cx/TheSpiritEngine2v106.msi"
             
        # mv Aronium.Setup.exe GameInstaller.exe
        # mv X.rar gameinstaller.rar
        # mv pq6-2.zip gameinstaller.zip
             
...
   
# POL_SetupWindow_menu "$(eval_gettext 'Do you want to install a translation file ?.')" "$TITLE" "$(eval_gettext 'No')~$(eval_gettext 'Italian')~$(eval_gettext 'Portuguese')" "~"


if [ "$APP_ANSWER" == "No" ]; then

elif [ "$APP_ANSWER" == "$(eval_gettext 'Italian')" ]; then
POL_SetupWindow_message "$(eval_gettext 'Italian.')" "$TITLE"
        cd "$WINEPREFIX/drive_c"
POL_SetupWindow_message "$(eval_gettext 'Download.')" "$TITLE"
        POL_Download "http://www.thespiritengine.com/Files/TSE2Translations/TSE2-TranslationPack-IT.zip"
POL_SetupWindow_message "$(eval_gettext 'Unzip.')" "$TITLE"
        POL_System_unzip "TSE2-TranslationPack-IT.zip" -d "$WINEPREFIX/drive_c/Program Files/The Spirit Engine 2/"

elif [ "$APP_ANSWER" == "$(eval_gettext 'Portuguese')" ]; then

        cd "$WINEPREFIX/drive_c"
        POL_Download "http://www.thespiritengine.com/Files/TSE2Translations/traducao_tse2.zip"
        POL_System_unzip "traducao_tse2.zip" -d "$WINEPREFIX/drive_c/Program Files/The Spirit Engine 2/"
...
iF0EABECAB0WIQRFtWEU2eoWQNaBNczlMfrJqhPKRwUCYCrhYQAKCRDlMfrJqhPK
R0m+AJ0f68vQzFsmLZvx2lk47BhqgDJ/NgCgrAAzwlF6Ruqhka7XfLz2KM6GTu0=
=tjBr
-----END PGP SIGNATURE-----
