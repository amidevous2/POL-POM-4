...
LNG_INSTALL_FIX="Téléchargement de l'installeur de cubanraul..."
...
LNG_INSTALL_FIX="Downloading cubanraul's installer..."
...
POL_GetSetupImages "http://files.playonlinux.com/resources/setups/bloodomen/top.jpg" "http://files.playonlinux.com/resources/setups/bloodomen/left.jpg" "$TITLE"
POL_SetupWindow_InitWithImages
 
POL_SetupWindow_presentation "$TITLE" "Silicon Knights" "http://www.siliconknights.com/" "GNU_Raziel" "$PREFIX" 
...
if [ "$GAME_MEDIAVERSION" == "DVD" ]; then
	...
	POL_SetupWindow_check_cdrom "kain.ico"
	# Download updated installer from http://home.comcast.net/~heavyweights/games/kain/index.htm
	cd "$POL_USER_ROOT/ressources"
	if [ ! -e "KainSetup.exe" ]; then
		POL_SetupWindow_download "$LNG_INSTALL_FIX" "$TITLE" "http://home.comcast.net/~masterpisgreat/KainSetup.exe"
	fi
	cd "$WINEPREFIX"/dosdevices
	ln -s "$CDROM" d:
	wine start /unix "$POL_USER_ROOT/ressources/KainSetup.exe"
...
## Fix for this game
# Fix save issue
GAME_PATH=`find $WINEPREFIX -name "KAIN.EXE" | sed s/KAIN.EXE//g`
cd "$GAME_PATH"
mkdir SAVED
...
# Making shortcut
POL_SetupWindow_auto_shortcut "$PREFIX" "KAIN.EXE" "$TITLE" "$TITLE.png" ""
...
iEYEABECAAYFAk4XUk0ACgkQ5TH6yaoTykdTkgCfZeIPN/b6TWy5XBApsPCTuqeM
qKMAn0ysecbla4eyVa+Z9VpOTWpd4NZ9
=+sWc
-----END PGP SIGNATURE-----
